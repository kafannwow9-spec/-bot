import { SlashCommandBuilder, ChatInputCommandInteraction, PermissionFlagsBits, ActionRowBuilder, StringSelectMenuBuilder, ComponentType } from 'discord.js';
import fs from 'fs';
...
export async function execute(interaction: ChatInputCommandInteraction) {
    ...
    const options = warnings[target!.id].map((w: any, index: number) => ({
        label: `تحذير ${index + 1}`,
    ...
    collector.on('collect', async i => {
        if (i.customId === 'remove_warning') {
            ...
            currentWarnings[target!.id] = currentWarnings[target!.id].filter((w: any) => w.id !== warningId);
